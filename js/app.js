/* ============================================
   Data Science Academy - Main Application
   ============================================ */
...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('ds-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('ds-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('ds-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('ds-progress', JSON.stringify(this.progress));
        localStorage.setItem('ds-quiz-results', JSON.stringify(this.quizResults));
    },
        ...
        let html = `
            <div class="fade-in">
                <div class="dashboard-hero">
                    <h2>Data Science Academy へようこそ</h2>
                    <p>データサイエンスを基礎からプロレベルまで体系的に学べる総合教育プラットフォームです。</p>
                </div>
    ...
    showCompletionMessage() {
        ...
        modal.innerHTML = `
            ...
            <p>
                全モジュールを完了しました！<br>
                あなたはデータサイエンスの基礎から実践まで一通り学びました。<br>
                実際のプロジェクトで経験を積んで、データサイエンティストを目指しましょう！
            </p>
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('ds-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('ds-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('ds-progress');
            localStorage.removeItem('ds-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
